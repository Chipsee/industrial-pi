...
static s8 gtp_io_test(struct i2c_client *client)
{
    ...
    {
        GTP_INFO("IC is GT928");
        bgt9271=TRUE;
    } else if (buf[2] =='9' && buf[3] =='2' && buf[4] =='7') {
        GTP_INFO("IC is GT9271");
        bgt9271=TRUE;
    } else
    {
	GTP_INFO("IC is GT911.");
        bgt9271=FALSE;
    }
    //We should use dtb to add io for diffrent board.
#ifdef CHANGEIO
...
static int gtp_fb_notifier_callback(struct notifier_block *noti, unsigned long event, void *data)
{
	...
	if (ev_data && ev_data->data && event == FB_EVENT_BLANK && ts) {
		...
		if (*blank == FB_BLANK_UNBLANK) {
			...
			goodix_ts_suspend(ts);  // Don't enter suspend mode.
		}
	}
...
static void gtp_early_suspend(struct early_suspend *h)
{
	...
	if (ts) {
		GTP_INFO("Suspend by earlysuspend module.");
		goodix_ts_suspend(ts);
	}
}
static void gtp_early_resume(struct early_suspend *h)
{
	...
	if (ts) {
		GTP_INFO("Resume by earlysuspend module.");
		goodix_ts_resume(ts);
	}	
}
#endif
