...
static int es8388_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	iface = snd_soc_component_read32(codec, ES8388_IFACE);
	adciface = snd_soc_component_read32(codec, ES8388_ADC_IFACE);
	daciface = snd_soc_component_read32(codec, ES8388_DAC_IFACE);
				...
				struct snd_soc_dai *dai)
{
	struct snd_soc_component *codec = dai->component;
	struct es8388_priv *es8388 = snd_soc_component_get_drvdata(codec);
	u16 srate = snd_soc_component_read32(codec, ES8388_IFACE) & 0x80;
	u16 adciface = snd_soc_component_read32(codec, ES8388_ADC_IFACE) & 0xE3;
	u16 daciface = snd_soc_component_read32(codec, ES8388_DAC_IFACE) & 0xC7;
	int coeff;
